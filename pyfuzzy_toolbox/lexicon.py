...
class SentiWords(BaseLexicon):
    ...
    def __read_lexicon_source(self):
        ...
        for line in file_obj.readlines():
            ...
            lemma = lemma_pos[0]
            pos = lemma_pos[1]
            prior_polarity_score = float(
                lemma_pos_prior_polarity_score[1].split("\n")[0])
            list_of_dict_units.append(
                {"lemma": lemma, "pos": pos, "prior_polarity_score": prior_polarity_score})
...
class SubjectivityClues(BaseLexicon):
    ...
    def __read_lexicon_source(self):
        ...
        for line in file_obj.readlines():
            clue = line.split()
            _type, _len, word1, pos1, stemmed1, priorpolarity = clue[0].split("=")[1], clue[1].split(
                "=")[1], clue[2].split("=")[1], clue[3].split("=")[1], clue[4].split("=")[1], clue[5].split("=")[1]
            list_of_dict_units.append(
                {"type": _type, "len": _len, "word1": word1, "pos1": pos1, "stemmed1": stemmed1, "priorpolarity": priorpolarity})
